	...
	return (
		<FlexWidget
			...
			<FlexWidget style={{ width: "match_parent", height: 32, flexDirection: "row", alignItems: "center" }}>
				...
				<FlexWidget style={{ width: 55, height: "match_parent", paddingHorizontal: 5, borderRadius: 9, backgroundColor: theme.primaryContainer, flexDirection: "row", alignItems: "center" }}>
					<TextWidget text={average} maxLines={1} truncate="END" style={{ color: theme.onPrimaryContainer, fontSize: 12, fontWeight: "700" }} />
					{averageSuffix ? <TextWidget text={averageSuffix} maxLines={1} style={{ color: theme.textMuted, fontSize: 6, fontWeight: "700", marginLeft: 1, marginTop: 3 }} /> : null}
				</FlexWidget>
				...
				{courses.length ? (
					<FlexWidget style={{ flex: 1, width: "match_parent", marginTop: 2 }}>
						<ListWidget style={{ height: "match_parent", width: "match_parent", backgroundColor: theme.transparent }}>
							{courses.map((course, index) => <CourseRow key={`${course.id || course.startMillis}-${index}`} course={course} active={index === 0} theme={theme} />)}
						</ListWidget>
					</FlexWidget>
				) : (
...
function CourseRow({ course, active, theme }: { course: WidgetCourse; active: boolean; theme: CourseWidgetTheme }) {
	...
	return (
		<FlexWidget
			...
			style={{ width: "match_parent", height: 24, marginBottom: 2, paddingHorizontal: 1, borderRadius: 7, backgroundColor: active ? theme.primaryContainer : theme.transparent, flexDirection: "row", alignItems: "center" }}>
