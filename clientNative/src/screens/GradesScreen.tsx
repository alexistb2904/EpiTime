...
function formatLastSync(value: string | null) {
	...
	if (Number.isNaN(date.getTime())) return "Derniere sync inconnue";
	return `Sync ${date.toLocaleDateString("fr-FR", { day: "2-digit", month: "2-digit" })} a ${date.toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" })}`;
}
...
export default function GradesScreen() {
	...
	const startMicrosoftLogin = async () => {
		...
		try {
			...
			Alert.alert("Connexion Auriga impossible", error instanceof Error ? error.message : "La connexion a echoue.");
		} finally {
			setConnecting(false);
		}
	};
	...
	const completeWebLogin = async (url: string) => {
		...
		try {
			...
			Alert.alert("Connexion Auriga impossible", error instanceof Error ? error.message : "Le code Auriga n'a pas pu etre valide.");
		} finally {
			...
			<ScrollView
				...
				<Animated.View entering={FadeInUp.duration(340)} style={s.headerRow}>
					<View style={s.headerText}>
						<Text style={[s.eyebrow, { color: theme.accent }]}>AURIGA EPITA</Text>
						<Text style={[s.title, { color: theme.text }]}>Notes</Text>
						<Text style={[s.lastSync, { color: theme.muted }]}>{formatLastSync(lastSync)}</Text>
					</View>
				...
				{offline ? (
					<Animated.View entering={FadeInDown.duration(260)} style={[s.offline, { backgroundColor: theme.accentSoft, borderColor: theme.border }]}>
						...
						<Text style={[s.offlineText, { color: theme.text }]}>Donnees hors ligne. Le cache reste affiche.</Text>
					</Animated.View>
				) : null}
				{selectedPeriod?.hasManualGrades ? (
					<Animated.View entering={FadeInDown.duration(260)} style={[s.manualNotice, { backgroundColor: theme.surface, borderColor: theme.warn }]}>
						...
						<Text style={[s.manualNoticeText, { color: theme.text }]}>Cette moyenne inclut des notes ajoutees manuellement.</Text>
					</Animated.View>
				) : null}
				{!connected ? (
					<ConnectCard
				...
				<Animated.View entering={FadeInDown.delay(40).duration(360)}>
					<Card style={s.averageCard} glow={false} accent>
						<View>
							...
							<Text style={[s.averageMeta, { color: theme.muted }]}>{useWeightedAverages ? "Moyenne ponderée" : "Moyenne non ponderée"}</Text>
						</View>
						<GraduationCap color={theme.accent} size={34} />
					</Card>
				</Animated.View>
				<View style={[s.segment, { backgroundColor: theme.surfaceSoft }]}>
					{(["notes", "syllabus"] as GradesMode[]).map((item) => {
				...
				<View style={[s.searchBox, { backgroundColor: theme.surface, borderColor: theme.border }]}>
					...
					<TextInput
						...
						placeholder={mode === "notes" ? "Rechercher une matiere" : "Rechercher un syllabus"}
				...
				{mode === "notes" ? (
					<View style={s.list}>
						{noteUes.length ? (
							noteUes.map(({ ue, subjects: ueSubjects }, groupIndex) => (
								<Animated.View
									key={ue.id}
									...
									<View style={s.ueHeader}>
										...
										<View style={s.ueHeaderText}>
											...
											<Text style={[s.ueMeta, { color: theme.muted }]}>
												...
												{ue.subjects.reduce((total, subject) => total + subject.grades.length, 0)} note
												{ue.subjects.reduce((total, subject) => total + subject.grades.length, 0) > 1 ? "s" : ""}
											</Text>
										</View>
							...
							<EmptyState text="Aucune matiere pour ce filtre." />
						)}
					</View>
				) : (
					<View style={s.list}>
						{groupedSyllabus.length ? (
							groupedSyllabus.map(([ue, items]) => (
...
function AuthHeader() {
	...
	return (
		<View style={s.header}>
			<Text style={[s.eyebrow, { color: theme.accent }]}>AURIGA EPITA</Text>
			<Text style={[s.title, { color: theme.text }]}>Notes EPITA</Text>
		</View>
	);
}
	...
	return (
		<Card style={[s.connectCard, compact && s.connectCardCompact]} accent={!compact}>
			...
			<Text style={[s.connectTitle, compact && s.connectTitleCompact, { color: theme.text }]}>
				{compact ? "Reconnecte Auriga pour synchroniser les donnees." : "Connecte Auriga pour afficher tes notes, moyennes et syllabus."}
			</Text>
...
function SubjectCard({ ue, subject, index, onPress, nested }: { ue: DisplayUE; subject: DisplaySubject; index: number; onPress: () => void; nested?: boolean }) {
	const { theme } = useTheme();
	return (
...
function SubjectDetailModal({
	...
	const averageStatus = subject?.hasNonValidated ? "A valider" : subject?.studentAverage ? "Calculee" : "En attente";
	...
	return (
		<Modal visible={Boolean(subject)} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
			<View style={[s.panelRoot, { backgroundColor: theme.bg, paddingTop: insets.top + 12 }]}>
				<PanelHeader title={subject?.name || "Matiere"} eyebrow={subject?.ueCode || "Notes"} onClose={onClose} />
				<ScrollView contentContainerStyle={[s.panelContent, { paddingBottom: insets.bottom + 38 }]}>
					<View style={[s.subjectHero, { backgroundColor: theme.surface, borderColor: subject?.hasManualGrades ? theme.warn : theme.border }]}>
						<View style={s.subjectHeroTop}>
							...
							<View style={s.heroCopy}>
								<Text style={[s.heroLabel, { color: theme.muted }]}>Moyenne matiere</Text>
						...
						<View style={s.heroStats}>
							...
							<View style={[s.heroStat, { backgroundColor: theme.bg, borderColor: theme.border }]}>
								<Target color={subject?.hasNonValidated ? theme.danger : theme.accent} size={17} />
								<Text style={[s.heroStatValue, { color: subject?.hasNonValidated ? theme.danger : theme.text }]}>{averageStatus}</Text>
					...
					{showAdd ? (
						<Animated.View entering={FadeInDown.duration(220)} style={[s.addPanel, { backgroundColor: theme.surface, borderColor: theme.border }]}>
							<Text style={[s.panelSectionTitle, { color: theme.text }]}>Assigner a un examen</Text>
							...
							<Pressable style={[s.idSubmitButton, { backgroundColor: theme.accent }]} onPress={saveManual}>
								<Text style={s.primaryButtonText}>Ajouter a la moyenne</Text>
							</Pressable>
						</Animated.View>
					) : null}
...
function GradeDetailModal({ grade, onClose, onDeleteManual }: { grade: DisplayGrade | null; onClose: () => void; onDeleteManual: (id: string) => Promise<void> }) {
	const { theme } = useTheme();
	const insets = useSafeAreaInsets();
	return (
